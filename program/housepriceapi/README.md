...
```bash
housepriceapi$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
housepriceapi$ sam local start-api
housepriceapi$ curl http://localhost:3000/classify_digit
```
...
```bash
housepriceapi$ sam logs -n InferenceFunction --stack-name "housepriceapi" --tail
```
...
```bash
sam delete --stack-name "housepriceapi"
```
